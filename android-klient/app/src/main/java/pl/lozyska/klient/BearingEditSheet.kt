...
@Composable
fun BearingEditSheet(vm: AppViewModel, bearing: Bearing?, onDismiss: () -> Unit, onSaved: () -> Unit) {
    ...
    var selectedShelf by remember { mutableStateOf(if (bearing?.reczny_przydzial == true) bearing.regal_id ?: AUTO_SHELF else AUTO_SHELF) }
    ...
    ModalBottomSheet(onDismissRequest = onDismiss, sheetState = sheetState) {
        Column(
            ...
            TextButton(onClick = {
                vm.lookupByDimensions(d.toDoubleOrNull(), D.toDoubleOrNull(), B.toDoubleOrNull()) { candidates ->
                    ...
                    if (first == null) {
                        ...
                        symbol = first.symbol
                        first.d?.let { d = fmtInput(it) }
                        first.dZew?.let { D = fmtInput(it) }
                        first.B?.let { B = fmtInput(it) }
                        first.typ?.let { typ = it }
                        source = if (first.online == true) "internet" else "offline"
                        note = if (candidates.size > 1) "Inne pasujące: ${candidates.drop(1).take(5).joinToString(", ") { c -> c.symbol }}"
                        else if (first.online == true) "Propozycja z internetu - zweryfikuj przed zapisem." else null
                    }
                }
            ...
            Spacer(Modifier.height(10.dp))
            ShelfDropdown(shelves = vm.shelves, selectedId = selectedShelf, onSelected = { selectedShelf = it })
            ...
            Row(Modifier.fillMaxWidth().padding(top = 18.dp), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                if (bearing != null) {
                    OutlinedButton(onClick = {
                        vm.deleteBearing(bearing.id) { onSaved() }
                ...
                Button(
                    onClick = {
                        val payload = BearingPayload(
                            symbol = symbol.trim(), typ = typ,
                            d = d.toDoubleOrNull(), dZew = D.toDoubleOrNull(), B = B.toDoubleOrNull(),
                            ilosc = ilosc.toIntOrNull() ?: 0, zrodlo = source, uwagi = uwagi.trim(),
                            regal_id = if (selectedShelf == AUTO_SHELF) null else selectedShelf,
                            reczny_przydzial = selectedShelf != AUTO_SHELF,
                        )
                        vm.saveBearing(bearing?.id, payload) { onSaved() }
                    },
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun ShelfDropdown(shelves: List<Shelf>, selectedId: Int, onSelected: (Int) -> Unit) {
    var expanded by remember { mutableStateOf(false) }
...
private fun shelfLabel(s: Shelf): String {
    val lo = s.d_min?.let { fmtInput(it) } ?: "0"
    val hi = s.d_max?.let { fmtInput(it) } ?: "∞"
